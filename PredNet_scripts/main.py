...
def read_image(path):
    ...
    top = int(args.offset[1] + (image.shape[1]  - args.size[1]) / 2)
    left = int(args.offset[0] + (image.shape[2]  - args.size[0]) / 2)
    bottom = int(args.size[1] + top)
    right = int(args.size[0] + left)
